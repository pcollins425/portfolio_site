...
def _system_prompt(session: dict[str, Any]) -> str:
    ...
    return (
        ...
        "- performance_index: {casino_id?, month_end:YYYY-MM-DD} — when user says "
        "'this month' / 'this month's report' with no month name, use the prior calendar "
        "month-end (e.g. on 2026-09-21 → 2026-08-31)\n"
        ...
        "(set month_end; default prior month-end for 'this month')\n"
        "- tell me about the performance for {month} → performance_index (processed only; no $)\n"
        "- compare months / how is performance / coin-in / win / commission → unsupported "
        "(Finance views; Ask AI has no dollar/trend verbs)\n"
        "- what reports can you generate / email a report → unsupported "
        "(report generate/send not live yet)\n"
        ...
        f"Clarify lane ids (optional hint; live only): {lanes}\n"
